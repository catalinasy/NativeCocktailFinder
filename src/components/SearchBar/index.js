...
import { Button, TextInput, TouchableOpacity, View, Image } from "react-native";
  ...
  handleTextChange = async keyword => {
    if (keyword.length >= 3) {
      ...
      if (drinks !== undefined && drinks !== null)
        this.props.setDrinks(formatDrinks(drinks));
    ...
    return (
      <View style={editable ? styles.list_search : styles.container}>
        <TouchableOpacity
          ...
          style={editable ? styles.list_wrapper : styles.wrapper}
        >
          <Image
          ...
          <TextInput
            ...
            style={
              editable && showCancel
                ? styles.fullInput
                : editable
                ? styles.fullInputNoCancel
                : styles.input
            }
            placeholder={
              editable ? "Search" : "Search your favourite cocktails"
            }
        ...
        {editable && showCancel && (
          <Button title="cancel" color="#D14842" onPress={this.handleCancel} />
        )}
      </View>
    );
  }
...
export default connect(
  null,
  mapDispatchToProps
)(withNavigation(SearchBarComponent));
